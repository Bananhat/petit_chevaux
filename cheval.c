...
void deplacement_test(char plateau[][15], joueur * p_j, int val_D, joueur liste_joueur[])
{
	...
	int *final = &p_j->liste_chevaux[n_cheval-1].final;
		// if cheval[n_cheval] est dans la liste des chevaux actifs
	...
	if (*final != 1 && *final != 2)
	{
		...
		if (valide == 0) // si il ne peut pas se deplacer
			{
		...
		else //sinon on lui demande si il est sur de vouloir se deplacer en simulant son deplacement
			{
				...
				{
					if(joueur_valide_deplacement(plateau, liste_joueur) && valide) //si c'est bon on fait les tests et on déplace
					{
						printf("Deplacement réalisé avec succès ! \n");
					}
					else // sinon on remet à 7 la où la simulation à été faites, et on le deplace a la position de départ
					{
